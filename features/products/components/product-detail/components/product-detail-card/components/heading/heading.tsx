...
// types
import { productHeading } from '@/types/product/components/product-detail/components/product-detail-card/components/heading/types';
// utils
import { formatPersianNumber } from '@/utils/numberFormatter';

const Heading: FC<productHeading> = (props) => {
  return (
    <div className="w-full">
      <div className="flex items-center justify-between gap-4">
        <h1 className="text-title-s lg:text-title-l font-semibold text-(--color-primary-950) w-[80%]">
          {props.title}
        </h1>
        <div className="w-fit py-[4px] px-[8px] rounded-(--radius-m) bg-[var(--brightens-700)] flex items-center gap-[4px]">
          <svg
            width="20"
            height="20"
            viewBox="0 0 20 20"
          ...
          <span>{formatPersianNumber(props.rating)}</span>
        </div>
      </div>
    </div>
  );
