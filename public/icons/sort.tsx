import { FC } from 'react';

interface Props {
  color?: string;
}

const SortIcon: FC<Props> = ({ color = '#3D3D3D' }) => {
  return (
    <svg
      width="100%"
      height="100%"
      viewBox="0 0 16 16"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path
        d="M14 5.1665H2C1.72667 5.1665 1.5 4.93984 1.5 4.6665C1.5 4.39317 1.72667 4.1665 2 4.1665H14C14.2733 4.1665 14.5 4.39317 14.5 4.6665C14.5 4.93984 14.2733 5.1665 14 5.1665Z"
        fill={color}
      />
      <path
        d="M12 8.5H4C3.72667 8.5 3.5 8.27333 3.5 8C3.5 7.72667 3.72667 7.5 4 7.5H12C12.2733 7.5 12.5 7.72667 12.5 8C12.5 8.27333 12.2733 8.5 12 8.5Z"
        fill={color}
      />
      <path
        d="M9.33464 11.8335H6.66797C6.39464 11.8335 6.16797 11.6068 6.16797 11.3335C6.16797 11.0602 6.39464 10.8335 6.66797 10.8335H9.33464C9.60797 10.8335 9.83464 11.0602 9.83464 11.3335C9.83464 11.6068 9.60797 11.8335 9.33464 11.8335Z"
        fill={color}
      />
    </svg>
  );
};
export default SortIcon;
